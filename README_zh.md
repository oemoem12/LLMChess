...
> 不需要联网、不需要 GPU 云端、不花一分钱——只要一个本地的 Ollama/llama.cpp/LM Studio，就能和 AI 下棋。
...
所以我写了 **LLM Chess**——一个用 Python + PyQt6 开发的国际象棋桌面应用，可以连接本地的 LLM 服务，实现人机对弈。
...
- **右侧**：走棋历史（SAN 格式）+ 当前 FEN 状态 + 游戏信息
- **底部**：New Game / Settings / Undo 三个按钮
...
2. **空响应**：偶尔 LLM 会返回空内容，代码里做了回退处理（自动选第一个合法着法）
...
│   ├── llm_connector.py       # OpenAI 兼容 API 连接器
│   ├── game_controller.py     # 游戏控制器 + 主窗口
...
每步棋，程序会向 LLM 发送这样的提示：

```
You are a chess engine. Play chess against a human user.

Current board FEN: rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR
Legal moves available: e2e4, e2e3, d2d4, d2d3, g1f3, b1c3, ...

Your move (ONLY the UCI string, no other text):
```

LLM 需要返回一个 UCI 格式的着法（如 `e2e4`）。代码里做了多层解析——正则匹配、模糊匹配、逐行检查，实在不行就 fallback 到第一个合法着法。
...
- [ ] 让 LLM 输出着法"理由"（赛后复盘时可以看）
- [ ] 支持多模型切换（比如开局用 Stockfish，中残局用 LLM）
- [ ] 保存 PGN 文件
